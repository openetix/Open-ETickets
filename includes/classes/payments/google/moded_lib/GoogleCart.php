...
//      New Digital Delivery Tags
        if($item->digital_content) {
          ...
          $xml_data->element('display-disposition',$item->display_disposition);
...
//          $ShippingPackage = $ship->ShippingPackage;
          ...
          if (!empty($ship->ShippingPackage->width) and !empty($ship->ShippingPackage->length)
                    and !empty($ship->ShippingPackage->height)) {
    ...
    function CheckoutServer2Server($proxy=array(), $certPath='') {
      ...
      $redirect = true;
      return $GRequest->SendServer2ServerCart($this->GetXML(), $redirect);
    }
